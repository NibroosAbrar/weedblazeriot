...
with col_wheel:
    ...
    if st.button("⬆️", use_container_width=True, key="wheel_up"):
        st.write("Moving Forward")
    col1, col2 = st.columns(2)
    with col1:
        if st.button("⬅️", use_container_width=True, key="wheel_left"):
            st.write("Turning Left")
    with col2:
        if st.button("➡️", use_container_width=True, key="wheel_right"):
            st.write("Turning Right")
    if st.button("⬇️", use_container_width=True, key="wheel_down"):
...
with col_manipulator:
    ...
    if st.button("⬆️", use_container_width=True, key="manip_up"):
        st.write("Moving Up")
    col3, col4 = st.columns(2)
    with col3:
        if st.button("⬅️", use_container_width=True, key="manip_left"):
            st.write("Moving Left")
    with col4:
        if st.button("➡️", use_container_width=True, key="manip_right"):
            st.write("Moving Right")
    if st.button("⬇️", use_container_width=True, key="manip_down"):
        st.write("Moving Down")

# Laser Control (Below Camera)
st.subheader("🔫 Laser Control")
if st.button("🔥 Fire Laser", use_container_width=True, key="laser_fire"):
